...
这是第一次打开仓库时建议优先阅读的实操文档。本文从准备硬件开始，适合边做边发现问题，目标是完成：

```text
准备硬件
安装 Ubuntu 虚拟机
同步 OpenHarmony 3.2 Release
合并 qihang 适配层
首次 hb build 编译成功
HiBurn 烧录
Xshell 串口日志验证
...
Windows 共享目录：C:\OpenHarmony\share
...
```text
C:\OpenHarmony\share\ubuntu_test.txt
```
...
```text
C:\OpenHarmony\share\Hi3861_wifiiot_app_allinone.bin
```
...
```text
../../archive/docs/01_通用配置步骤_原始记录.md
```
