...
这里记录的是完整 dashboard 参考实现，主要用于代码参考、对照和故障排查。建议先完成 MQTT 命令行验证，再基于 [simple-dashboard 模板](../../templates/simple-dashboard/README.md) 编写或改造前端。

完整 dashboard 不算第一次联调的必需步骤。它连接的是 Windows 上的 Mosquitto Broker，不是直接连开发板。
...
设备端先按 [编译烧录步骤](./编译烧录指南.md) 烧录成功，再按 [MQTT 测试步骤](./MQTT测试指南.md) 确认 MQTT 通路正常。需要参考完整实现时，再看这篇。
