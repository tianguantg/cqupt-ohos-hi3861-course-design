...
void GUI_ShowFont16_UTF(u8 x,u8 y,u8 *s,u8 mode)
{
	u8 i,j,k,tmp;
	u16 num;
	num = sizeof(cfont16_UTF)/sizeof(typFNT_GB16_UTF);
    for(i=0;i<num;i++)
	{
		if((cfont16_UTF[i].Index[0]==*s) && (cfont16_UTF[i].Index[1]==*(s+1)) && (cfont16_UTF[i].Index[2]==*(s+2)))
		{
			for(j=0;j<32;j++)
			{
				if(mode)
				{
					tmp = cfont16_UTF[i].Msk[j];
				}
				else
				{
					tmp = ~(cfont16_UTF[i].Msk[j]);
...
void GUI_ShowFont24_UTF(u8 x,u8 y,u8 *s,u8 mode)
{
	u8 i,j,k,tmp;
	u16 num;
	num = sizeof(cfont24)/sizeof(typFNT_GB24_UTF);
    for(i=0;i<num;i++)
	{
...
void GUI_ShowFont32_UTF(u8 x,u8 y,u8 *s,u8 mode)
{
	u8 i,j,k,tmp;
	u16 num;
	num = sizeof(cfont32)/sizeof(typFNT_GB32);
...
/* UTF-8编码中文占三个字节 */
void GUI_ShowCHinese_UTF(u8 x,u8 y,u8 hsize,u8 *str,u8 mode)
{ 
    dump_buf(str, 16);
	while(*str!='\0')
	{
		if(hsize == 16)
		{
			GUI_ShowFont16_UTF(x,y,str,mode);
		}
		else if(hsize == 24)
		{
			GUI_ShowFont24_UTF(x,y,str,mode);
		}
		else if(hsize == 32)
		{
			GUI_ShowFont32_UTF(x,y,str,mode);
		}
		else
		{
			return;
